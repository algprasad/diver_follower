...
void diverTrackingCallback(const darknet_ros_msgs::BoundingBoxesConstPtr& bounding_boxes_msg,
    ...
    diver_global_pose.header.stamp = ros::Time::now();
    pub_diver_pose.publish(diver_global_pose);
    std::cout<<diver_global_pose<<std::endl;
...
int main(int argc, char** argv){
    ros::init(argc, argv, "diver_measured_pose");
    ros::NodeHandle nh;
    pub_diver_pose = nh.advertise<geometry_msgs::PoseStamped>("hippocampus/diver_follower/diver_pose_measurement", 10);
    ...
    message_filters::Synchronizer<MySyncPolicy> sync2(MySyncPolicy(5), sub_bounding_box, sub_depth_points, sub_drone_pose);
